...
namespace http
{
    ...
    {
        //断言而false则直接代码终止 仅在调试模式下有效：assert 不适合用于生产环境的错误处理，因为它在发布模式下会被移除。
    ...
    void HttpRequest::setQueryParameters(const char* start,const char* end)
    {
        ...
        // 按 & 分割多个参数
        while((pos = std::string::find('&',prev)) != std::sring::npos)
        {
            std::string pair = argumentStr.substr(pos,pos-prev);
