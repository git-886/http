    ...
    namespace router
    {
        class Router
        {
            public:
                ...
                using HandlderCallback = std::function<void (const HttpRequest & ,HttpResponse *)>;
                //使用路由键和路由哈希可加快路由匹配效率
                // 路由键（请求方法 + URI）
                struct RouteKey
                {
                ...
                //registerHandler：注册对象式处理器（精准匹配）。registerCallback：注册回调函数式处理器（精准匹配）。// 注册路由处理器
            ...
            private:
                ...
                { // 将路径模式转换为正则表达式，支持匹配任意路径参数 通过正则分组获得具体参数
            ...
            private:
                ...
                //第一个参数：键类型  第二个参数：值类型  第三个参数：自定义哈希函数类型
